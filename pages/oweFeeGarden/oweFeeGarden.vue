<template>
	<view>
		<scroll-view scroll-y style="padding-bottom: 200rpx;">
			...
			<view class="cu-list menu"  v-for="(item,index) in feeRooms" :key="index" :data-item="item">
				<view class="cu-item " >
					<view class="content padding-tb-sm">
						<view>
							<view class="text-cut" style="width:150px">{{item.payerObjName}}</view>
						</view>
					</view>
					<view class="action">
				...
				getRoomOweFees(_objData)
					...
					.then(function(_fees){
						_fees.forEach(function(_item){
							...
							_that.hasInFeeRoom(_item);
						})
						_that.receivableAmount = _that.receivableAmount.toFixed(2);
					})
			},
			hasInFeeRoom:function(_fee){
				let _room = null;
				this.feeRooms.forEach(room =>{
					if(room.payerObjName == _fee.payerObjName){
						_room = room;
					}
				});
				
				if(!_room){
					this.feeRooms.push(_fee);
					return ;
				}
